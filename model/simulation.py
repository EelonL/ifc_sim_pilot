...
def run_simulation(
    ...
    for _, row in df.iterrows():
        ...
        # A delivery delay advances the schedule to a new working day. Any
        # unused capacity on the earlier day cannot be carried across the gap.
        if delivery_wait:
            current_day += delivery_wait
            capacity_left = daily_capacity
        ...
        # Pack small work packages into the same day while enforcing the
        # combined crew/crane capacity across package boundaries.
        while remaining > 0:
            if capacity_left <= 0:
                current_day += 1
                capacity_left = daily_capacity
            installed_today = min(remaining, capacity_left)
            remaining -= installed_today
            capacity_left -= installed_today
            work_days += 1

        installation_finish_day = current_day
        finish_day = installation_finish_day + rework_wait
        ...
        if rework_wait:
            # Rework consumes the remainder of its finish day. The next work
            # package therefore starts on the following day.
            capacity_left = 0
